...
class NewsRepositoryTest {
    ...
    // 业务码 200 + 两条数据的响应
    private val sampleResponseWithData =
        NewsResponseDto(
            code = 200,
            msg = "success",
            newsList =
    ...
    @Test
    fun `blank api key - returns Client 401 and skips network`() =
        runTest {
            // Given: apiKey 是空字符串
    ...
    @Test
    fun `business code 200 with data - returns Success list`() =
        runTest {
            // Given
    ...
    @Test
    fun `business code 200 with empty list - returns Success empty`() =
        runTest {
            // Given
            val repository = NewsRepository(api, apiKey = "valid-key", channel = "keji")
    ...
    @Test
    fun `business code non 200 - returns Error Client`() =
        runTest {
            // Given: 业务码 500
            val repository = NewsRepository(api, apiKey = "valid-key", channel = "keji")
    ...
    @Test
    fun `network error - falls back to fake data with isFromFallback`() =
        runTest {
            // Given: 网络失败
            val repository = NewsRepository(api, apiKey = "valid-key", channel = "keji")
