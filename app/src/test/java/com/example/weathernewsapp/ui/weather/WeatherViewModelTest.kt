...
class WeatherViewModelTest {
    ...
    @Test
    fun `init triggers load and state becomes Success`() =
        runTest(testDispatcher.scheduler) {
            stubRepositorySuccess()
            val vm = WeatherViewModel(repository)
            advanceUntilIdle()
    ...
    @Test
    fun `error response becomes Error state and sends toast event`() =
        runTest(testDispatcher.scheduler) {
    ...
    @Test
    fun `cache response sends offline toast event`() =
        runTest(testDispatcher.scheduler) {
            every { repository.defaultCityFlow } returns flowOf("北京")
            every { repository.tempUnitFlow } returns flowOf(TempUnit.CELSIUS)
